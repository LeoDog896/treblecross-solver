import { Number } from "https://deno.land/x/cliffy@v0.20.1/prompt/mod.ts";

import { keypress } from "https://deno.land/x/cliffy@v0.24.2/keypress/mod.ts";

import { Game, game, solve } from "./index.ts";
...
function print(game: Game) {
  const result = solve(game).map((x) => x == -length ? "" : x).map((x) =>
